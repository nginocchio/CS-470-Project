function getPatchesForDisease( diseaseLabel, patchWidth, patchHeight, shiftVal, usingWindows )
% INPUT: diseaseLabel:             string specifying a particular disease value
%	 patchWidth & patchHeight: integer denoting the extracted patch size (16, 32, 64)
%	 shiftVal:                 float, denotes frequency that patches will be extracted (.25, .5, 1.0)
%	 usingWindows:	           Boolean, specifies the users' operating system
%
% OUTPUT: New file directory with the diseaseLabel, and all extracted .png files with that label
%
% -------------------------------------------------------------------------  
...
% --------------------------------WARNING----------------------------------
%
% Any patches/images saved in the patient_patches folder will be
...
mainDataDir = 'patient_patches';
if ~exist (mainDataDir, 'dir')
    if usingWindows
        mkdir .\patient_patches
    else
        mkdir ./patient_patches
    end
end
...
if usingWindows
    diseaseDir = strcat( '.\patient_patches\', diseaseLabel );
else
    diseaseDir = strcat( 'patient_patches/', diseaseLabel );
end

if ~exist(diseaseDir, 'dir')
    mkdir(mainDataDir, diseaseLabel);
end

addpath(diseaseDir)

parentDir = strcat(diseaseDir, '/');
datasetDir = './';
...
diseaseLabel = char(diseaseLabel);
            ...
            for r = 1:((imheight/patchShift)-subBy)
                for c = 1:((imwidth/patchShift)-subBy)
                    ...
                    if ROIpixelCount >= areaPatch*patchQualification
                        ...
                        %ROIpatch = insertObjectAnnotation(Y, 'rectangle', [xmin ymin patchWidth-1 patchHeight-1], 'fibrosis');
                        %figure
